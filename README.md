...
docs/WHY_NOT.md         why a congruence sieve can never finish, with -1 as an
                        explicit witness
docs/EXPLORE.md         three explorations; one refuted its guess, one proved
                        a_(j+k) >= a_j a_k and made the tail bound rigorous
...
python/                 collatz_maxodd — cycle search, sieves, backward tree,
                        ...
                        the ledgers and the landing form; 426 tests
...
web/                    data.json (140 KB) + DATA.md — precomputed visualization
                        ...
                        generated by build_page.py from page.template.html; front.html —
                        the front door that indexes everything; and six standalone
                        pages, each a single file: rays.html, surface.html, atlas.html,
                        ledger.html, multiplicative.html, landing.html, drop.html (see "Pages").
```

## Pages

Everything interactive is a single HTML file under `web/`, published as a
private artifact. Start at the front door, which indexes the rest and says for
each page what it shows, what it establishes, and what it does not settle.

| page | file | what it is |
|---|---|---|
| [Still Not Solved](https://claude.ai/artifact/Ka93oeMYfWhb5aLUX2Kooi) | `web/front.html` | the front door |
| [The Largest Odd Number](https://claude.ai/artifact/LUiAkgnva2tjYNP1Bwwrgo) | `web/index.html` | the study itself: the sieve on cycle maxima, T0–T8, the census, the ladders, the words |
| [Halving Rays](https://claude.ai/artifact/EDwtTTYSLd9PNsDUP48R5E) | `web/rays.html` | rays `y·2^x`, landing pads, rise/fall |
| [Halving Surface](https://claude.ai/artifact/NspdyQPppeVkKowGHNiF3C) | `web/surface.html` | the rays in three dimensions, with trajectories |
| [Parity Atlas](https://claude.ai/artifact/CqRzRVAdrRVZd5SspEBFKK) | `web/atlas.html` | Terras's theorem as a table, coefficient stopping time |
| [Cycle Ledger](https://claude.ai/artifact/SnaWUkMG6NSKjCD1HYmX8m) | `web/ledger.html` | the additive balance, the gates at both ends, a SAT box solved in-page |
| [Multiplicative Ledger](https://claude.ai/artifact/TUdU2rAtKnP9q2HsFcKPa5) | `web/multiplicative.html` | `2^B = ∏(3 + 1/nᵢ)` against the continued fraction of `log₂3`; Hercher's floor; the circuit ladder |
