...
/-- `∑_{k < n} b_(k+1)` — total halvings over the first `n` backward hops. -/
def sumB (C : Cycle q) : Nat → Nat
  | 0     => 0
  | n + 1 => C.bb (n + 1) + C.sumB n
...
theorem prodG_eq : ∀ n, C.prodG n = 2 ^ C.sumB n * C.prodFrom 0 n := by
  intro n
  induction n with
  | zero => simp [prodG, sumB, prodFrom]
  | succ n ih =>
      show (3 * C.y (n + 1) + q) * C.prodG n
         = 2 ^ (C.bb (n + 1) + C.sumB n) * (C.y (0 + n) * C.prodFrom 0 n)
...
theorem pow_le_of_min {m : Nat} (hm : ∀ i, m ≤ C.y i) :
    m ^ C.L * 2 ^ C.sumB C.L ≤ (3 * m + q) ^ C.L := by
  have h := C.prod_squeeze hm C.L
  rw [C.prodG_eq C.L, C.prodFrom_one_eq_zero] at h
  have hp := C.prodFrom_pos 0 C.L
  have h' : (m ^ C.L * 2 ^ C.sumB C.L) * C.prodFrom 0 C.L
          ≤ ((3 * m + q) ^ C.L) * C.prodFrom 0 C.L := by
    calc (m ^ C.L * 2 ^ C.sumB C.L) * C.prodFrom 0 C.L
        = m ^ C.L * (2 ^ C.sumB C.L * C.prodFrom 0 C.L) := by simp [Nat.mul_comm, Nat.mul_left_comm]
...
theorem length_bound {m κ c : Nat}
    ...
    (hsmall : 2 * C.L * q ≤ 3 * m)
    (hbaker : 3 ^ C.L * C.L ^ κ + c * 3 ^ C.L ≤ 2 ^ C.sumB C.L * C.L ^ κ) :
    3 * m * c ≤ 2 * q * C.L ^ (κ + 1) := by
  have sq := C.squeeze hm hmpos hsmall
  have h1 : 3 * m * (3 ^ C.L * C.L ^ κ + c * 3 ^ C.L)
          ≤ 3 * m * (2 ^ C.sumB C.L * C.L ^ κ) := Nat.mul_le_mul_left _ hbaker
  have h2 : 3 * m * (2 ^ C.sumB C.L * C.L ^ κ)
          ≤ 3 ^ C.L * (3 * m + 2 * C.L * q) * C.L ^ κ := by
    calc 3 * m * (2 ^ C.sumB C.L * C.L ^ κ)
        = (3 * m * 2 ^ C.sumB C.L) * C.L ^ κ := by
          simp [Nat.mul_comm, Nat.mul_left_comm, Nat.mul_assoc]
...
theorem cycle5_length_bound : 3 * 19 * 5 ≤ 2 * 5 * cycle5.L ^ (3 + 1) := by
  ...
  · show 2 * 3 * 5 ≤ 3 * 19
    decide
  · show 3 ^ 3 * 3 ^ 3 + 5 * 3 ^ 3 ≤ 2 ^ cycle5.sumB 3 * 3 ^ 3
    rw [cycle5_sumB]
    decide
